...
from cmd import rotate, rotate1, rotate2, reset, stop, is_pressed, run, read_color, color_off, rotate_t
...
MOVE_DEG = 130 # アームを90度回すのに必要な回転角度
...
class Control:
    ...
    def move_inv(self, motor_num1, deg1, motor_num2, deg2):
        ...
        waitdeg = max(abs(deg1), abs(deg2)) - 50
    ...
    def test_execute(self, solution, direction):
        ...
        for i in range(len(rot)): # rot分だけ回す
            if len(rot[i]) > 3:
                self.rotate_inv(rot[i][0], rot[i][1], rot[i][2], rot[i][3])
                self.arm_set(rot[i][0])
                self.arm_set(rot[i][2])
            else:
                if rot[i][0] < 4: # [i][_]が0~3ならキューブを回す
                    self.rotate(rot[i][0], rot[i][1])
                    self.arm_set(rot[i][0])
